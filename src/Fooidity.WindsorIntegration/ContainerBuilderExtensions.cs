namespace Fooidity
{
    ...
    public static class ContainerBuilderExtensions
    {
        ...
        public static void RegisterSwitched<TFeature, T>(
            this IWindsorContainer container, Func<IKernel, T> enabledFactory, Func<IKernel, T> disabledFactory)
            where TFeature : struct, CodeFeature
            where T : class
         
        {
            container.Register(Component.For<T>().UsingFactoryMethod(k =>
            {
        ...
        /// <param name="container">The container container</param>
        /// <returns>The registration container for the container, already configured for the specified types</returns>
        public static void RegisterSwitchedType<TFeature, T, TEnabled, TDisabled>(this IWindsorContainer container)
            where TFeature : struct, CodeFeature
            where T : class
